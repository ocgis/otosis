...
static INLINE double to_single (UInt32 value)
{
    double frac;
...
static INLINE UInt32 from_single (double src)
{
    int expon;
    UInt32 tmp;
    double frac;
...
static INLINE double to_exten(UInt32 wrd1, UInt32 wrd2, UInt32 wrd3)
{
    double frac;
...
static INLINE void from_exten(double src, UInt32 * wrd1, UInt32 * wrd2, UInt32 * wrd3)
{
    int expon;
    double frac;
    ...
    *wrd2 = (UInt32) (frac * 4294967296.0);
    *wrd3 = (UInt32) (frac * 18446744073709551616.0 - *wrd2 * 4294967296.0);
}

static INLINE double to_double(UInt32 wrd1, UInt32 wrd2)
{
    double frac;
...
static INLINE void from_double(double src, UInt32 * wrd1, UInt32 * wrd2)
{
    int expon;
    int tmp;
    double frac;
	...
	*wrd1 = 0;
    ...
    tmp = (UInt32) (frac * 2097152.0);
    *wrd1 |= (((expon + 1023 - 1) & 0x7ff) << 20) | (tmp & 0xfffff);
    *wrd2 = (UInt32) (frac * 9007199254740992.0 - tmp * 4294967296.0);
}

static INLINE double to_pack(UInt32 wrd1, UInt32 wrd2, UInt32 wrd3)
{
    double d;
    char *cp;
    char str[100];
...
static INLINE void from_pack(double src, UInt32 * wrd1, UInt32 * wrd2, UInt32 * wrd3)
{
    int i;
    int t;
    char *cp;
...
static INLINE int get_fp_value (UInt32 opcode, UInt16 extra, double *src)
{
    Ptr32 tmppc;
    UInt16 tmp;
    int size;
	...
	*src = regs.fp[(extra >> 10) & 7];
	...
	switch (size) {
	case 6:
	    *src = (double) (SInt8) m68k_dreg (regs, reg);
	    break;
	case 4:
	    *src = (double) (SInt16) m68k_dreg (regs, reg);
	    break;
	case 0:
	    *src = (double) (SInt32) m68k_dreg (regs, reg);
	    break;
	case 1:
	...
	break;
    case 5:
	ad = m68k_areg (regs, reg) + (SInt32) (SInt16) next_iword();
	break;
    case 6:
    ...
    switch (size) {
    case 0:
	*src = (double) (SInt32) get_long (ad);
	break;
    case 1:
	*src = to_single(get_long (ad));
	break;
    case 2:{
	    UInt32 wrd1, wrd2, wrd3;
	    wrd1 = get_long (ad);
	    ad += 4;
	    wrd2 = get_long (ad);
	    ad += 4;
...
static INLINE int put_fp_value (double value, UInt32 opcode, UInt16 extra)
{
    UInt16 tmp;
    Ptr32 tmppc;
    int size;
...
static INLINE int get_fp_ad(UInt32 opcode, UInt32 * ad)
{
    UInt16 tmp;
    Ptr32 tmppc;
    int mode;
...
static INLINE int fpp_cond(UInt32 opcode, int contition)
{
...
void fdbcc_opp(UInt32 opcode, UInt16 extra)
{
    Ptr32 pc = (UInt32) m68k_getpc ();
    SInt32 disp = (SInt32) (SInt16) next_iword();
    int cc;
...
void fscc_opp(UInt32 opcode, UInt16 extra)
{
    UInt32 ad;
    int cc;
...
void ftrapcc_opp(UInt32 opcode, Ptr32 oldpc)
{
    int cc;
...
void fbcc_opp(UInt32 opcode, Ptr32 pc, UInt32 extra)
{
    int cc;
	...
	if ((opcode & 0x40) == 0)
	    extra = (SInt32) (SInt16) extra;
	m68k_setpc (pc + extra);
    }
}

void fsave_opp(UInt32 opcode)
{
    UInt32 ad;
...
void frestore_opp(UInt32 opcode)
{
    UInt32 ad;
    UInt32 d;
    int incr = (opcode & 0x38) == 0x20 ? -1 : 1;
...
void fpp_opp(UInt32 opcode, UInt16 extra)
{
    int reg;
    double src;
